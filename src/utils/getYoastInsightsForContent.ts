...
const IgnoredAssessments = ['UrlLengthAssessment', 'TaxonomyTextLengthAssessment'];
    ...
    (acc, [category, assessments]) => {
      Object.entries(assessments).forEach(([name, value]) => {
        if (!IgnoredAssessments.includes(name)) {
          if (typeof value === 'object' && value?.hasOwnProperty('getResult')) {
            ...
            const configKey = get(AssessmentClassConfigKeyMap, name, null) as
              | string
              | null
              | undefined;

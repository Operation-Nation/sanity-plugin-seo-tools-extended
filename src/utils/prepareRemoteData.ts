...
export async function prepareRemoteData(url: URL, options: Options): Promise<PreparedData> {
  ...
  const title =
    doc.title || doc.querySelector('h1')?.textContent || doc.querySelector('h2')?.textContent || '';
  ...
  await Promise.all(Array.from(nodeList).map(n => createElement.appendChild(n)));

...
const SeoToolsPaneView: React.FC<Props> = ({ document }) => {
  ...
  const seoData = useSWR(
    ['seoData', document._id, document._rev, config],
    async ([key, id, rev, config]) => {
  ...
  return (
    <Box padding={3}>
      {seoData.isValidating && <SeoToolsAnalyzingCard />}

      {seoData.error && <SeoToolsErrorCard>{extractErrorMessage(seoData.error)}</SeoToolsErrorCard>}

      {seoData.data && (
        <Stack space={3}>
          {config.render(
            seoData.data.seo,
            seoData.data.prepared,
